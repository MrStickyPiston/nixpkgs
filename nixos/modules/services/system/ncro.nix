...
let
  ...
  fallbackPublicKeys = optional (cfg.settings.fallback_cache.enabled or false) (
  ...
  upstreamPublicKeys = lib.pipe ((cfg.settings.upstreams or [ ]) ++ fallbackPublicKeys) [
    (builtins.map (upstream: if builtins.isAttrs upstream then upstream.public_key or "" else upstream))
...
in
{
  options.services.ncro = {
    ...
    package = lib.mkPackageOption pkgs "ncro" { };
